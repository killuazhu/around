...
func checkUser(username, password string) bool {
	...
	searchStr := strings.Split(username, "-")[0]
	termQuery := elastic.NewTermQuery("username", searchStr)
	...
	for _, item := range queryResult.Each(reflect.TypeOf(tyu)) {
		...
		fmt.Println("In the query iteration: U.Username:U.Password is", u.Username, u.Password)
		fmt.Println("In the query iteration: ussername:password is", username, password)
